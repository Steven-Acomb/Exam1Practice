...
def problem3a(window, point, n):
    ...
    sum = 0
    for k in range(n):
        point1 = rg.Point(point.x + n * 20, point.y - n*10)
        point2 = rg.Point(point.x + n * 20, point.y - 50 - n * 10)
...
def problem3b(m, point1):
    ...
    sum = 0
    for k in range(m):
        point = rg.Point(point1.x, point1.y - 75 * k)
        sum += problem3a(window,point,k+1)
    return sum
